var handleRenderDatepicker = function () {
  ...
  $("#datepicker-default").datepicker({ autoclose: true ,language: "fa",
    weekStart: 0,  });
  $("#datepicker-component").datepicker({ autoclose: true ,language: "fa",
    weekStart: 0 });
  $("#datepicker-range").datepicker({ autoclose: true ,language: "fa",
    weekStart: 0 });
  $("#datepicker-inline").datepicker({ autoclose: true ,language: "fa",
    weekStart: 0 });
...
//         monthNames: moment.localeData()._jMonthsShort,
...
var handleRenderBootstrapSlider = function () {
  ...
  $("#slider-tooltip").bootstrapSlider({ tooltip: "always" });
  $("#slider-vertical").bootstrapSlider({ reversed: true });
...
var handleRenderjQueryFileUpload = function () {
  ...
  if ($.support.cors) {
    $.ajax({
      ...
      var alert =
        '<div class="alert alert-danger m-b-0 m-t-15">آپلود سرور در حال حاضر در دسترس نیست - ' +
        new Date() +
        "</div>";
